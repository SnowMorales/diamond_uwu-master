			...
			elements[#elements+1] = {
				...
				onSelect = function()
					local amount = lib.inputDialog(TranslateCap('withdraw_amount'), {
						{type = 'number', label = TranslateCap('amount_title'), description = TranslateCap('withdraw_amount_placeholder'), required = true, min = 1, max = 1000000}
						})
			...
			elements[#elements+1] = {
				...
				onSelect = function()
					local amount = lib.inputDialog(TranslateCap('deposit_amount'), {
						{type = 'number', label = TranslateCap('amount_title'), description = TranslateCap('deposit_amount_placeholder'), required = true, min = 1, max = 25000000}
						})
						if not amount then return end
